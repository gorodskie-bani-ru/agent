...
import { ReviewsPageView } from './View'
...
import { isReviewCompany } from 'src/Custom/interfaces'
...
export const ReviewsPage: Page<PageProps & { page: number }> = ({
  siteOrigin,
  page,
}) => {
  ...
  const reviews = useMemo(
    () => response.data?.concepts?.filter(isReviewCompany) ?? [],
...
ReviewsPage.getInitialProps = async ({ query, apolloClient }) => {
