...
import { AIChatPanel } from 'src/Custom/components/ai/AIChatPanel'
import { CompanyCard } from 'src/Custom/components/catalog/CompanyCard'
import { Pagination } from 'src/components/Pagination'
import { OwnerCTA } from 'src/Custom/components/catalog/OwnerCTA'
...
const Hero = styled.section`
  background:
    radial-gradient(
      ...
      ${({ theme }) => theme.lovable.colors.accent},
      transparent 60%
    ),
    radial-gradient(
...
const HeroInner = styled.div`
  ...
  @media (min-width: ${({ theme }) => theme.lovable.bp.lg}) {
...
const ToggleBtn = styled.button<{ $active: boolean }>`
  ...
  background: ${({ $active, theme }) =>
    $active ? theme.lovable.colors.primary : 'transparent'};
...
export const LayoutMapWrapper = styled.div`
  overflow: hidden;
  position: relative;
`

type LayoutProps = { $view: 'list' | 'map' }
      ...
      <Section>
        <Toolbar>
          ...
          <Toggle role="tablist">
            <ToggleBtn
              $active={view === 'list'}
              onClick={useCallback(() => setView('list'), [])}
            >
              <ListIcon size={14} /> Список
            </ToggleBtn>
            <ToggleBtn
              $active={view === 'map'}
              onClick={useCallback(() => setView('map'), [])}
            >
        ...
        <Layout $view={view}>
          ...
          {map && <LayoutMapWrapper>{map}</LayoutMapWrapper>}
        </Layout>
